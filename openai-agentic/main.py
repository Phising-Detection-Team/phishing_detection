...
class Orchestrator:
    ...
    def __init__(self, round_id: int, num_parallel_workflows: int = 2):
        ...
        print(f"[Orchestrator] Generator: Gemini 2.5 Flash")
        ...
        for i in range(num_emails):
            ...
            try:
                    ...
                    try:
                        ...
                        if '503' in error_str or 'UNAVAILABLE' in error_str.upper():
                            if retry_count < max_retries:
                                ...
                                print(f"[Workflow {workflow_id}] Email {i+1}: Generator unavailable (503), retrying in {wait_time}s (attempt {retry_count}/{max_retries})")
                ...
                if gen_result is None:
                    raise ValueError('Generator failed after all retries')
                ...
                email_id = save_generated_email(
                    ...
                    prompt="Generated by Gemini 2.5 Flash",
                    subject=subject,
                    body=body,
                ...
                # Debug: Print detector response
                if det_result and hasattr(det_result, 'final_output'):
                    print(f"[Detector Debug] Response preview: {det_result.final_output[:200]}...", file=sys.stderr)
                else:
                    print(f"[Detector Debug] No final_output attribute in result", file=sys.stderr)
                
                # Parse detector output
                det_output = self._parse_json_output(det_result.final_output if det_result else "")
                
                if not det_output:
                    # Graceful failure: save detection with default safe values
                    print(f"[Workflow {workflow_id}] Email {i+1} returned invalid JSON, saving with defaults...")
                    det_tokens = self._estimate_tokens(det_result.final_output if det_result else "")
                    det_cost = self._estimate_cost(det_tokens, 'claude-3-haiku-20240307')
                    processing_time = time.time() - email_start_time
                    
                    # Save with conservative defaults (mark as non-phishing to avoid false positives)
                    is_correct = update_email_with_detection(
                        email_id=email_id,
                        verdict='legitimate',  # Conservative default when detection fails
                        confidence=0.5,  # Low confidence when detection fails
                        risk_score=0.5,
                        reasoning='Detection failed - marked as legitimate by default',
                        llm_provider='claude',
                    ...
                    results['emails_succeeded'] += 0  # Don't count as success due to failure
                    results['emails_failed'] += 1
                    
                    print(f"[Workflow {workflow_id}] Email {i+1}: Detection failed (marked as legitimate by default)")
                    continue
                
                # Extract detection data
                verdict_raw = det_output.get('verdict', 'legitimate').lower()
                
                # Normalize verdict to schema constraint: 'phishing' or 'legitimate'
                verdict = verdict_raw if verdict_raw in ['phishing', 'legitimate'] else 'legitimate'
                ...
                reasoning = det_output.get('reasoning', 'Analysis completed')
    ...
    def _parse_json_output(self, output: str) -> Dict[str, Any]:
        ...
        try:
            if not output:
                print(f"[Parser] ERROR: Empty output received", file=sys.stderr)
                return {}
            ...
            print(f"[Parser] Successfully parsed JSON with keys: {list(result.keys())}", file=sys.stderr)
            return result
            
        except json.JSONDecodeError as e:
            print(f"[Parser] JSON decode error: {e}", file=sys.stderr)
            print(f"[Parser] Cleaned output (first 300 chars): {cleaned[:300] if 'cleaned' in locals() else 'N/A'}", file=sys.stderr)
            print(f"[Parser] Raw output (first 300 chars): {output[:300]}", file=sys.stderr)
            return {}
        except Exception as e:
            print(f"[Parser] Unexpected error: {e}", file=sys.stderr)
            return {}
