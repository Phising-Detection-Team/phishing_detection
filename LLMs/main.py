...
from backend.app.models import db, Round, Email # Import models
...
async def main():
    ...
    for round_num in range(1, num_rounds + 1):
        ...
        # Update Round Record with results
        db_round.status = 'completed'
        db_round.completed_at = datetime.now(UTC)
        db_round.total_cost = total_round_cost
        db.session.commit()
